%%%%%%%%%%%%%  Function main %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Purpose:  
%      Detect smallest and largest disks in the given input image
%
% Input Variables:
%       NONE
%      
...
%      5.  Removal of sal and peppar noise using closing function
%      6.  Apply hit transform to detect smallest disks in the image
%      7.  Apply miss transform to detect largest disks in the image
%
%  Restrictions/Notes:
...
saveas(figure(2),[pwd '\output_images\2Input Image(16 bits).tiff']);
...
saveas(figure(3),[pwd '\output_images\3Image with one intensity channel.tiff']);
...
saveas(figure(4),[pwd '\output_images\4Thresholded Grayscale Image.tiff']);
...
saveas(figure(5),[pwd '\output_images\5Negative of Thresholded Grayscale Image.tiff']);

% Creation of a structuring element (straight line of length 3 pixels and thickness 1 pixel) 
% for salt and pepper noise removal using opening and closing
se_noise_removal = [1 1 1];
figure(6), imshow(se_noise_removal)
title('Structuring element to remove sal and pepper noise')
saveas(figure(6),[pwd '\output_images\6Structuring element to remove sal and pepper noise.tiff']);

% Salt and peppar noise is present throughout the image.
% Closing is performed to remove salt and peppar noise present in the image
closed_image = closing(opened_image, se_noise_removal);
img_without_noise = closed_image;
figure(7), imshow(img_without_noise)
title('Image after removing salt and pepper noise using Closing')
saveas(figure(7),[pwd '\output_images\7Image after removing salt and pepper noise using Closing.tiff']);
...
se_smalldisk = img_without_noise(46:68, 101:128);
figure(8), imshow(se_smalldisk)
title('Structuring element to detect small disks')
saveas(figure(8),[pwd '\output_images\8Structuring element to detect small disks.tiff']);

%Step2: Remove small disks from the input image
erode_smalldisk = dilation_erosion(img_without_noise,se_smalldisk,2);
figure(9), imshow(erode_smalldisk)
title('Small disks are eroded from the whole image')
saveas(figure(9),[pwd '\output_images\9Small disks are eroded from the whole image.tiff']);
...
saveas(figure(10),[pwd '\output_images\10Remaining disks are dilated to retrieve thir original shape and size.tiff']);

%Step4: Remove all the remaining disks from the input image to get an
%output image containing only smallest disks
smalldisks_output = img_without_noise - retrieve_disks;
figure(11), imshow(smalldisks_output)
title('Image containing only smallest disks')
saveas(figure(11),[pwd '\output_images\11Image containing only smallest disks.tiff']);
...
largedisk_withnbh = img_without_noise(391-x:455+x, 461-x:530+x);
figure(12), imshow(largedisk_withnbh)
title('Large Disk')
saveas(figure(12),[pwd '\output_images\12Large Disk.tiff']);
...
saveas(figure(13),[pwd '\output_images\13Structuring element to detect large disks.tiff']);

%For miss transform, the complement of the input image is used to perform
%the transforms
neg_image = ~img_without_noise;
...
saveas(figure(14),[pwd '\output_images\14Erode large disks from input image.tiff']);
...
saveas(figure(15),[pwd '\output_images\15Disks are dilated to retrieve thir original shape and size.tiff']);
...
saveas(figure(16),[pwd '\output_images\16Image containing only largest disks.tiff']);
...
saveas(figure(17),[pwd '\output_images\17Image containing only largest disks.tiff']);
